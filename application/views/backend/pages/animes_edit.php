<?php
	$animes_detail = $this->db->get_where('animes',array('animes_id'=>$animes_id))->row();
?>
<div class="row">
    <div class="col-6">
        <div class="card">
            <div class="card-body">
				<form method="post" action="<?php echo base_url();?>index.php?admin/animes_edit/<?php echo $animes_id;?>" enctype="multipart/form-data">
	                <div class="form-group mb-3">
	                    <label for="title"><?php echo get_phrase('Animes_Title'); ?></label>
	                    <input type="text" class="form-control" id = "title" name="title" value="<?php echo $animes_detail->title;?>">
	                </div>
					...
					<div class="form-group mb-3">
						...
						<textarea class="form-control" id="description_short" name="description_short" rows="6"><?php echo $animes_detail->description_short;?></textarea>
					</div>

					<div class="form-group mb-3">
						<label for="description_long"><?php echo get_phrase('Long_description'); ?></label>
						<textarea class="form-control" id="description_long" name="description_long" rows="6"><?php echo $animes_detail->description_long;?></textarea>
					</div>
					...
					<div class="form-group mb-3">
						...
						<select class="form-control select2" id="genre_id" name="genre_id">
							...
							<option
								<?php if ( $animes_detail->genre_id == $row2['genre_id']) echo 'selected';?>
					...
					<div class="form-group mb-3">
						...
						<select class="form-control select2" id="year" name="year">
							...
							<option
								<?php if ( $animes_detail->year == $i) echo 'selected';?>
								value="<?php echo $i;?>">
								<?php echo $i;?>
							</option>
							<?php endfor;?>
					...
					<div class="form-group mb-3">
						...
						<select class="form-control select2" id="rating" name="rating">
							<?php for ($i = 0; $i <= 5 ; $i++):?>
							<option
								<?php if ( $animes_detail->rating == $i) echo 'selected';?>
								value="<?php echo $i;?>">
								<?php echo $i;?>
							</option>
							<?php endfor;?>
					...
					<div class="row">
						<div class="form-group col-6">
							<input type="submit" class="btn btn-success col-12" href="<?php echo base_url();?>index.php?admin/animes_list" value="<?php echo get_phrase('update_anime')?>">
						</div>
						<div class="col-6">
	...
	<div class="col-6">
		<div class="card">
            <div class="card-body">
				...
				<a href="<?php echo base_url();?>index.php?admin/season_create/<?php echo $animes_id;?>"
				...
				<table class="table table-hover table-centered mb-0" width="100%">
					<tbody>
						<?php
							$seasons	=	$this->crud_model->get_seasons_of_animes($animes_id);
						...
						<tr>
							...
							<td>
								<a href="<?php echo base_url();?>index.php?admin/season_edit/<?php echo $animes_id.'/'.$row['season_id'];?>"
									class="btn btn-info btn-xs btn-mini">
								<?php echo get_phrase('manage_episodes'); ?></a>
								<a href="<?php echo base_url();?>index.php?admin/season_delete/<?php echo $animes_id.'/'.$row['season_id'];?>"
