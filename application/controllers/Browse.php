...
class Browse extends CI_Controller {
...

	{
		...
		if($called_function == 'search' || $called_function == 'process_list' || $called_function == 'home' ||
		  		...
		  		$called_function == 'movie' || $called_function == 'mylist' || $called_function == 'series' || $called_function == 'animes' ||

		  		$called_function == 'playmovie' || $called_function == 'playseries' || $called_function == 'playanimes') {
...

	{
		...
		$page_data['page_title']	=	'Watch Series';
...

	{
		...
		$page_data['page_name']		=	'animes';

		$page_data['page_title']	=	'Watch Animes';
        ...
        $total_result = $this->db->count_all_results('animes');
        ...
        $page_data['animes'] = $this->crud_model->get_animes($genre_id , $per_page, $this->uri->segment(4));
	...
	function playanimes($animes_id = '', $season_id = '', $episode_id = "")

	{
		...
		if ($episode_id == "") {
			...
			$episodes	=	$this->db->get_where('animes_episode', array('season_id'=>$page_data['season_id']))->row_array();
		...
		$page_data['page_name']		=	'playanimes';
